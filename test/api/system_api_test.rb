...
class Qatime::SystemApiTest < ActionDispatch::IntegrationTest
  ...
  test 'check_update api' do
    get "/api/v1/system/check_update", {category: '教师问答社区', version: '0.0.1', platform: 'windows'}
  ...
  test 'check update api enforce false' do
    get "/api/v1/system/check_update", {category: '教师问答社区', version: '0.0.2', platform: 'windows'}
    assert_response :success
    res = JSON.parse(response.body)
    assert_equal 1, res['status']
    assert !res['data']['enforce']
