  ...
  class VideoCourseBillingDirectorTest < ActiveSupport::TestCase
    ...
    test 'test billing a video course' do
      ...
      assert_difference "Payment::BillingItem.count", 5, '账单条目数量不正确' do
        assert_difference "@system_account.reload.balance.to_f", -270, "系统收入不正确" do
          assert_difference "@publish_account.reload.balance.to_f", 15, "发行商收入不正确" do
            assert_difference "@sell_account.reload.balance.to_f", 75, "销售收入不正确" do
              assert_difference "@teacher_account.reload.balance.to_f", 180, "教师收入不正确" do
