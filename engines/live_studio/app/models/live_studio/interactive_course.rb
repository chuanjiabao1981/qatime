module LiveStudio
  class InteractiveCourse < ActiveRecord::Base
    ...
    validates :name, presence: { message: "请输入直播名称" }, length: { in: 2..20 }
    validates :description, presence: { message: "请输入直播介绍" }, length: { in: 5..300 }
    validates :grade, presence: { message: "请选择年级" }
    validates :subject, presence: { message: "请选择科目" }
    ...
    validates :teacher_percentage, presence: { message: "请填写老师分成" }, numericality: { only_integer: true, greater_than_or_equal_to: 0, less_than_or_equal_to: 100 }
    ...
    validates :interactive_lessons, presence: {message: '请添加至少一节课程'}
