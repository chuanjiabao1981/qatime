  ...
  class OrdersController < ApplicationController
    ...
    def create
      ...
      if @order.save && !@order.failed?
        redirect_to payment.pay_user_order_path(current_user, @order.transaction_no)
      elsif @order.failed?
        redirect_to payment.user_order_path(current_user, @order.transaction_no), alert: t("flash.alert.order_failed")
      else
        p @order.errors
        p '--------------'
        render :new
