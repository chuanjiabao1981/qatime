...
module LiveStudio
  class VideoLesson < ActiveRecord::Base
    ...
    scope :unstart, -> { where('status < ?', VideoLesson.statuses[:teaching]) } # 未开始的课程
    scope :should_complete, -> { where(status: [Lesson.statuses[:finished], VideoLesson.statuses[:billing]]).where("class_date < ?", Date.yesterday)} # 可以completed的课程
    scope :teached, -> { where("status > ?", VideoLesson.statuses[:closed]) } # 已经完成上课, 不可以继续直播的课程才算完成上课
    ...
    scope :waiting_finish, -> { where(status: [VideoLesson.statuses[:paused], VideoLesson.statuses[:closed]])}
    scope :month, -> (month){where('live_studio_video_lessons.class_date >= ? and live_studio_video_lessons.class_date <= ?', month.beginning_of_month.to_date,month.end_of_month.to_date)}
    scope :started, -> { where("status >= ?", VideoLesson.statuses[:teaching])} # 已开始
    scope :readied, -> { where("status >= ?", VideoLesson.statuses[:ready])} # 已就绪
    ...
    def replays_for(user)
      return false if user.nil?
      return true if user.admin?
      video_course.buy_tickets.where(student_id: user.id).available.exists?
    end
    ...
    def merge_replays
      ...
      replays.create(video_for: ChannelVideo.video_fors['board'],
                     ...
                     channel: video_course.channels.find_by(use_for: Channel.use_fors['board']))
    end
    ...
    def increment_course_counter(attribute)
      video_course.increment(attribute)
    end
  end
end
