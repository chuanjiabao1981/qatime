...
sumitVideo = (token,videoFile)->
  ...
  videoForm = """
    <form id="videos_upload" action="/videos" enctype="multipart/form-data" method="post">
    </form>
  """
  videoFormData = new FormData
  videoFormData.append "video[token]", token
  videoFormData.append "video[name]" , videoFile
...
((factory) ->
  ...
  createVideoNode = (id) ->
    v = $('<iframe>').attr('frameborder', 0).attr('src', "//localhost:3000/videos/#{id}")
    v[0]
