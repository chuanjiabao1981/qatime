module LiveStudio
  class Seller::CoursesController < Seller::BaseController
    ...
    def create
      ...
      if @course.save
        redirect_to seller_course_path(@current_resource, @course), notice: i18n_notice('created', @course)
      else
        @workstations = workstations
        render :new
      end
    ...
    def update
      if @course.update(course_params)
        redirect_to seller_course_path(@current_resource, @course), notice: i18n_notice('updated', @course)
      else
        render :edit
      end
    end
