  ...
  class TeacherPermission < UserPermission
    ...
    def initialize(user)
      ...
      allow 'live_studio/teacher/courses', [:index, :show, :edit, :update, :sync_channel_streams, :channel, :close, :update_class_date] do |teacher, course, action|
        # 只有初始化的辅导班可以编辑 | 非完成状态可以调课
        permission = (%w(edit update).include?(action) ? course.init? : true) || (%w(update).include?(action) ? (course && !course.completed?) : true)
      ...
      allow 'live_studio/teacher/lessons', [
        ...
        # 课程的辅导班初始化状态可以变新增 编辑 删除 | 招生中状态可以编辑 | 非完成状态可以调课
        permission = (%w(new create edit update destroy).include?(action) ? course.init? : true) || (%w(edit update).include?(action) ? course.preview? : true) || (%w(create update).include?(action) ? !course.completed? : true)
        teacher && teacher == user && permission
      end
