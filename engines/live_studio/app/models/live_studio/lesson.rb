...
module LiveStudio
  class Lesson < ActiveRecord::Base
    ...
    scope :unclosed, -> { where('live_studio_courses.status < ?', Lesson.statuses[:closed]) } # 未关闭的课程
    scope :already_closed, -> { where('live_studio_courses.status >= ?', Lesson.statuses[:closed]) } # 已关闭的课程
