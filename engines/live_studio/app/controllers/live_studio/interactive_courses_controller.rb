...
module LiveStudio
  class InteractiveCoursesController < ApplicationController
    ...
    def interactive_courses_params
      params[:interactive_course][:interactive_lessons_attributes] = params[:interactive_course].to_unsafe_hash[:interactive_lessons_attributes].map(&:second) if params[:interactive_course] && params[:interactive_course][:interactive_lessons_attributes]
    ...
    def build_preview_course
      ...
      if params[:interactive_course][:interactive_lessons_attributes].blank?
        ...
        course.interactive_lessons_count = params[:interactive_course].to_unsafe_hash[:interactive_lessons_attributes].try(:count) || 0
        class_dates = params[:interactive_course].to_unsafe_hash[:interactive_lessons_attributes].map { |a| a[:class_date] if a[:_destroy] == '0' }.reject(&:blank?)
      end
      course.class_date = class_dates.min
      course.start_at = class_dates.min
      course.end_at = class_dates.max
      course
    end
