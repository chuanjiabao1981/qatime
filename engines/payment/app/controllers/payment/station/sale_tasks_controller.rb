...
module Payment
  class Station::SaleTasksController < Station::BaseController
    ...
    def index
      # 未结束
      @waiting_tasks = SaleTask.unclosed
    ...
    # POST /sale_tasks
    def create
      @sale_task = SaleTask.new(sale_task_params)

      if @sale_task.save
        redirect_to @sale_task, notice: 'Sale task was successfully created.'
      else
        render :new
      end
    end
    ...
    def update
      if @sale_task.update(sale_task_params)
        redirect_to @sale_task, notice: 'Sale task was successfully updated.'
      else
        render :edit
      end
    end
