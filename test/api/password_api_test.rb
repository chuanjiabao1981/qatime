...
class Qatime::PasswordAPITest < ActionDispatch::IntegrationTest
  ...
  test "PUT /api/v1/users/:id/password find password by email" do
    user = users(:student1)
    login_account = user.email

    post "/api/v1/captcha", {send_to: login_account, key: :get_password_back}
    put "/api/v1/user/find_password", {login_account: login_account, captcha_confirmation: "1234", password: "pa123456", password_confirmation: "pa123456"}, 'Remember-Token' => @remember_token
  ...
  test "PUT /api/v1/users/user/find_password find password by login_mobile" do
    user = users(:student1)
    login_account = user.login_mobile

    post "/api/v1/captcha", {send_to: login_account, key: :get_password_back}

    put "/api/v1/user/find_password", {login_account: user.login_mobile,
