...
module Payment
  class UsersController < ApplicationController
    ...
    # 充值记录
    def recharges
      @recharges = query_by_date(@current_resource.cash_account.payment_recharges)
      @recharges = @recharges.order(created_at: :desc).paginate(page: params[:page])
    end

    # 提现记录
    def withdraws
      @withdraws = query_by_date(@current_resource.cash_account.payment_withdraws)
      @withdraws = @withdraws.order(created_at: :desc).paginate(page: params[:page])
    end

    # 消费记录
    def consumptione_records
      @withdraws = query_by_date(@current_resource.cash_account.consumption_records.includes(:target))
      @withdraws = @withdraws.order(created_at: :desc).paginate(page: params[:page])
    end

    # 收入记录
    def earning_records
      @earning_records = query_by_date(@current_resource.cash_account.earning_records.includes(:target))
      @earning_records = @earning_records.order(created_at: :desc).paginate(page: params[:page])
    end

    # 退款记录
    def refunds
      @refunds = query_by_date(@current_resource.cash_account.payment_refunds)
