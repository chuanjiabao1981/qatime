module LiveStudio
  class Group < ActiveRecord::Base
    ...
    has_many :live_sessions, through: :events
    has_many :live_studio_course_notifications, as: :notificationable, dependent: :destroy

    accepts_nested_attributes_for :events, allow_destroy: true, reject_if: proc { |attributes| attributes['_update'] == '0' }
    validates_associated :events
    validates :events, presence: { message: I18n.t('view.live_studio/course.validates.lessons') }

    has_many :students, through: :buy_tickets

    has_many :play_records, as: :product # 听课记录
    has_many :announcements, as: :announcementable
    has_many :qr_codes, as: :qr_codeable, class_name: "::QrCode"

    has_many :billings, through: :events, class_name: 'Payment::Billing' # 结算记录
    ...
    scope :published_start, -> { where('live_studio_groups.status > ?', statuses[:init]) }
    ...
    def live_next_time
      lesson = events.include_today.unstart.first
    ...
    def own_by?(user)
      ...
      user.live_studio_tickets.available.find {|t| t.product_id == id && t.product_type == 'LiveStudio::Group' }.present?
    end
    ...
    def tasting?(user)
      return false unless user.present?
      return false unless user.student?
      user.live_studio_taste_tickets.available.find {|t| t.product_id == id && t.product_type == 'LiveStudio::Group' }.present?
    end
    ...
    def reset_completed_lessons_count!
      teached_count = events.teached.count
    ...
    def current_lesson
      ...
      @current_lesson ||= events.find {|l| l.class_date.try(:today?) && l.unclosed? }
      @current_lesson ||= events.select {|l| l.class_date.try(:today?) }.last
      @current_lesson ||= events.find {|l| l.class_date > Date.today && l.unclosed? }
      @current_lesson ||= events.select {|l| l.class_date.present? }.last
    end
    ...
    def clear_lessons
      events.map(&:destroy) unless deleted_at.blank?
    end
    ...
    def lower_price
      ...
      lp = 0
      lp = events.size * 5 if events.size > 0
      lp
    end
  end
end
